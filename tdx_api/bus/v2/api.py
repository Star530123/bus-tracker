...
api_header_data: Dict[str, Any] = {}

def get_bus_real_time_near_stop(city: str, route: int, query: str) -> List[Dict[str, Any]]:
...
def get_bus_stop_of_route(city: str, route: int, query: str) -> List[Dict[str, Any]]:
...
def _api_header() -> Dict[str, str]:
    if len(api_header_data) == 0 or time.time() >= api_header_data['expired_time']:
        ...
        response: Dict[str, Any] = json.loads(authentication)
...
def _auth_api_header() -> Dict[str, str]:
