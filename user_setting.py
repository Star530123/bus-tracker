...
class UserSetting:
    def __init__(self, username, city, route, direction, target_stop, before_target_stop: tuple):
    ...
    def update_information(self, stops_of_route):
        same_direction_stops = next((route for route in stops_of_route if route.get('Direction') == self.direction), None)
